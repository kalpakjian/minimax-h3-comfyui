# -*- coding: utf-8 -*-
"""Minimal H3 T2V: hard-coded int width/height/length. Saves via SaveImage (legacy).
Avoids ResolutionSelector / ComfyMathExpression / CreateVideo completely."""
...
G={
 ...
 "4":{"class_type":"MiniMaxH3TurboLoRA","inputs":{"model":["1",0],"lora_name":"minimax_h3_turbo_v4_step600_ema.safetensors","strength":1.0,"low_vram":False}},
 ...
 "12":{"class_type":"SaveImage","inputs":{"images":["11",0],"filename_prefix":"video/MiniMax_H3_5s_frames"}},
}
server_post=json.dumps({"prompt":G,"client_id":"h3-min"})
# str keys + str links everywhere consistent
def post(obj):
...
def get(u):
    with urllib.request.urlopen(u,timeout=30) as r:
        return json.loads(r.read().decode())
print("Submitting...")
resp=post({"prompt":G,"client_id":"h3-min"})
if "error" in resp:
