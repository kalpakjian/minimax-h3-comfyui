# -*- coding: utf-8 -*-
"""Robust hand-written 5s MiniMax-H3 T2V turbo workflow."""
...
PROMPT=("Cinematic shot: a sleek silver robot slowly walking through a rain-soaked "
 ...
 "pulse, subtle rain ambience. Moody, cinematic, cinematic lighting.")
...
G={
 ...
 15:{"class_type":"SaveVideo","inputs":{"video":[14,"0"],"filename_prefix":"video/MiniMax_H3_5s","format":"auto","codec":{"key":"auto","inputs":{"required":{}}}}},
}
...
def post(obj):
    ...
    try:
        ...
        print("HTTP",e.code,e.read().decode("utf-8","replace")[:2500])
        raise
...
json.dump(G,open(r"C:\minimax+comfyUI\h3_5s_api.json","w",encoding="utf-8"),indent=2)
print("Submitting...")
...
while time.time()-t0<timeout:
    ...
    if status=="error":
        for m in st.get("messages",[]):
            if m[0]=="execution_error": print("EXEC ERR:",str(m[1].get("exception_message",""))[:2500])
        break
    if st.get("completed") or status=="success": break
else:
    print("TIMEOUT");sys.exit(1)
print("STATUS",status,"%.1fs"%(time.time()-t0))
print("outputs:",json.dumps(h.get("outputs",{}),ensure_ascii=False)[:1500])
if status!="success":
    for m in st.get("messages",[]): print("MSG",m[0],str(m[1])[:500])
    sys.exit(1)