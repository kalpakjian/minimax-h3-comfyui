# -*- coding: utf-8 -*-
"""5s MiniMax-H3 T2V turbo -> SaveWEBM video + SaveAudio. Bypasses VIDEO-type nodes."""
...
PROMPT=("Cinematic shot: a sleek silver robot slowly walking through a rain-soaked "
 ...
 "pulse, subtle rain ambience. Moody, cinematic lighting.")
...
G={
 ...
 14:{"class_type":"SaveWEBM","inputs":{"images":[12,"0"],"filename_prefix":"video/MiniMax_H3_5s","codec":"vp9","fps":24.0,"crf":15}},
...
def post(obj):
    ...
    try:
        ...
        print("HTTP",e.code,e.read().decode("utf-8","replace")[:2500]);raise

def get(u):
    with urllib.request.urlopen(u,timeout=30) as r:
        return json.loads(r.read().decode())

json.dump(G,open(r"C:\minimax+comfyUI\h3_5s_webm_audio.json","w",encoding="utf-8"),indent=2)
print("Submitting...")
...
if status!="success":
    for m in st.get("messages",[]):print("MSG",m[0],str(m[1])[:600])
    sys.exit(1)